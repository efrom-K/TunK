...
impl TlsSniffer {
    ...
    pub fn analyze_tls_handshake(&self, data: &[u8]) -> Result<Option<String>, String> {
        ...
        let content_length = u16::from_le_bytes([data[3], data[4]]) as usize;
        
        if data.len() < 5 + content_length {
        ...
        let tls_version = u16::from_le_bytes([data[client_hello_start], data[client_hello_start + 1]]);
            ...
            if extension_type == 0x0000 || extension_type == 0x0017 {
                ...
                if offset + 4 + length <= data.len() {
                    ...
                    if data[offset + 4] == 0x01 { // SNI List
                        let list_length = u16::from_le_bytes([data[offset + 5], data[offset + 6]]) as usize;
                        
                        if offset + 7 + list_length <= data.len() {
                            let name_list_offset = offset + 7;
                            
                            while name_list_offset < data.len() && 
                                  name_list_offset + 3 <= data.len() {
                                let name_type = u16::from_le_bytes([data[name_list_offset], data[name_list_offset + 1]]);
                                
                                if name_type == 0x00 { // Host Name
                                    let name_length = u16::from_le_bytes([data[name_list_offset + 2], data[name_list_offset + 3]]) as usize;
                                    ...
                                    if name_list_offset + 4 + name_length <= data.len() {
                                        ...
                                        if domain_end <= data.len() {
                                            ...
                                            // Декодируем домен (UTF-8)
                                            match String::from_utf8(domain_bytes.clone()) {
                                                Ok(domain) => {
                                    ...
                                    let _ = u16::from_le_bytes([data[name_list_offset + 2], data[name_list_offset + 3]]);
                                }
                                
                                offset += 4;
                            }
                        }
                    } else if name_type == 0x01 { // SNI List (IP Address)
                        let _ = u16::from_le_bytes([data[offset + 2], data[offset + 3]]);
                        offset += 4;
                    } else {
                        break;
                    }
                ...
                if offset + 4 + length <= data.len() {
                    ...
                    while list_offset < data.len() && 
                        ...
                        let name_type = u16::from_le_bytes([data[list_offset], data[list_offset + 1]]);
                        
                        if name_type == 0x00 { // Host Name
                            let name_length = u16::from_le_bytes([data[list_offset + 2], data[list_offset + 3]]) as usize;
                            ...
                            let _ = u16::from_le_bytes([data[list_offset + 2], data[list_offset + 3]]);
                        }
    ...
    pub fn sniff_packet(&self, packet_offset: u64, domain: String) -> Result<(), String> {
...
mod tests {
    ...
    fn test_tls_record_type_detection() {
        ...
        let tls_header = vec![0x16, 0x03, 0x03, 0x00, 0x2e];
    ...
    fn test_tls_version_check() {
        ...
        // TLS 1.2 (0x0303)
        let tls_1_2_header = vec![0x16, 0x03, 0x03, 0x00, 0x2e];
    ...
    fn test_old_tls_version() {
        ...
        // TLS 1.0 (0x0301) - should be accepted
        let tls_1_0_header = vec![0x16, 0x03, 0x01, 0x00, 0x2e];
    ...
    fn test_unsupported_tls_version() {
        ...
        // TLS 1.3 (0x0304) - should be accepted
        let tls_1_3_header = vec![0x16, 0x03, 0x04, 0x00, 0x2e];
